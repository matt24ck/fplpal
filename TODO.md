...
- [ ] **Multi-GW transfer planner** (by GW2, PLAN §5) — multi-period MILP over 5–8 GWs: FT banking, −4 hits, sell-price rules. New engine module + `optimize_transfers` tool + REST endpoint + Planner UI transfer cards ("−4 now, +6.2 expected over 4 GWs", alternatives with deltas).
