...
/** Reads ?team= in its own Suspense island — useSearchParams() opts its
 * caller out of static prerendering, and it must not take the table (the
 * page's crawlable content) down with it. */
function TeamFilterReader({ onTeam }: { onTeam: (team: string | null) => void }) {
...
function Explorer() {
  ...
  const slugs = useMemo(() => (data ? assignSlugs(data.players) : null), [data]);
  ...
  return (
    <PageShell title="Players">
      <Suspense fallback={null}>
        <TeamFilterReader onTeam={setTeamFilter} />
      </Suspense>
                ...
                <td className="bg-chalk group-hover:bg-paper-2 sticky left-0 px-3 py-1.5">
                  <span className="flex items-center gap-2">
                    ...
                    <span className="min-w-0">
                      {/* real anchor: the profile pages need crawlable links */}
                      <Link
                        href={`/players/${slugs?.get(p.code) ?? p.code}`}
                        onClick={(e) => e.stopPropagation()}
                        className="block max-w-40 truncate font-medium underline-offset-2 hover:underline"
                      >
                        {p.player}
                      </Link>
